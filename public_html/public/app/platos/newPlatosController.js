...
newPlatos.$inject = ['$http', '$scope', 'SystemUser', 'platosData', '$location', 'model', 'fileUpload', 'Notification', 'errorHandler', '$firebaseObject', 'platoDomain','userNombre'];

function newPlatos($http, $scope, SystemUser, dataService, $location, modelPlatos, FileUploader, Notification, errorHandler, $firebaseObject, platoDomain,userNombre) {
    var domain = 'platos';
    ...
    $scope.SavePlato = function () {
        if ($scope.platoForm.$valid) {
            ...
            dataPlato.nombre = $scope.nombre,
                    ...
                    dataPlato.usuarioNombre =  userNombre.nombre($scope.UserUid).name
